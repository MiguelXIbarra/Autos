    ...
    <div class="w-full max-w-md">
        ...
        <div class="bg-[#0D0D0D] border border-white/5 p-10 rounded-3xl shadow-2xl backdrop-blur-md bg-opacity-80">
            <div class="text-center mb-10">
                <h2 class="text-[0.6rem] text-[#C9A24A] font-black uppercase tracking-[0.6em] mb-4">Acceso Exclusivo</h2>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-8">
                ...
                {{-- Email --}}
                <div class="space-y-2">
                    <label class="text-[0.6rem] uppercase tracking-widest text-gray-500 font-bold">Identificación (Email)</label>
                ...
                {{-- Password --}}
                <div class="space-y-2">
                    <label class="text-[0.6rem] uppercase tracking-widest text-gray-500 font-bold">Código de Acceso (Password)</label>
                ...
                <div class="flex items-center justify-between pt-2">
                    <label class="flex items-center text-[0.6rem] uppercase tracking-widest text-gray-500 cursor-pointer hover:text-white transition">
                        <input type="checkbox" name="remember" class="mr-2 accent-[#C9A24A]"> Recordarme
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-[0.6rem] uppercase tracking-widest text-[#C9A24A] hover:text-white transition">¿Olvidó su clave?</a>
                    @endif
                </div>
        ...
        <div class="text-center mt-8">
            <p class="text-gray-500 text-[0.7rem] uppercase tracking-widest">
                ...
                <a href="{{ route('register') }}" class="text-white hover:text-[#C9A24A] transition ml-2">Solicitar Registro</a>
            </p>
        </div>
    </div>
</div>
